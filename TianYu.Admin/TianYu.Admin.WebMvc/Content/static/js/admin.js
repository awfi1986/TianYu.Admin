...
layui.define(['jquery', 'form', 'layer', 'element'], function (exports) {
    ...
    form.on('submit(login)', function (data) {
        ...
        var redUrl = param.redUrl;
        var userName = param.u;
        var passWord = param.p;

        $.ajax({
            type: 'post',
            url: '/Home/Api_Login',
            dataType: 'json',
            data: { u: userName, p: passWord, redUrl: redUrl },
            success: function (res) {
                layer.close(index);
                if (res.Status == 200) {
                    res = res.Data;
                    if (res.Status == 1) {
                        location.replace(res.BusinessData);
                    }
                    else {
     ...
     * 图片缩放和放大
     */
    $('body').on('click', '.tianyu-active-image', function () {
        $(this).toggleClass('tianyu-active-image-max');
    });

    /**
     * 表格列表图片弹出框显示
     */
    $('body').on('click', '.tianyu-table-list-image', function () {
        var url = $(this).attr('src');
        layer.open({
            type: 1,
            title: false,
            closeBtn: 0,
            skin: 'layui-layer-nobg tianyu-table-list-image-layer', //没有背景色
            shadeClose: true,
            content: '<img src="' + url + '">'
        });
    });
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.Status == 200) {
                        res = res.Data;
                        if (res.Status == 1) {
                            window.location.replace(res.BusinessData);
                        }
                        else {
                            layer.msg(res.ErrorMessage);
                        }
	...
	*/
    window.WeAdminShow = function (title, url, w, h) {
        ...
        if (url == null || url == '') {
            url = "/Error/Index?msg=程序出错，请联系系统管理员";
        };
        if (w == null || w == '') {
            w = ($(window).width() * 0.9);
        };
 ...
 * 切换提交按钮状态
 * @@param elem 按钮元素
 * @@param status 状态：true 启用，false 禁用，默认值 false
 */
var toggleSubmitStatus = function (elem, status) {
